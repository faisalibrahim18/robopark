...
export const DetailDevicePage = ({ match }) => {
  const { devicesId } = match.params;
  ...
  const handleCurrency = async () => {
    ...
    const {data} = await axios.get(`${API_URL}/api/v1/devices/${userInfo.business_id}`)
  ...
  const [devicesInitial, setDevicesInitial] = React.useState({
    name: "",
    email: "",
    phone_number: "",
    address: "",
  ...
  const [devicesHistory, setDevicesHistory] = React.useState([]);

  const DevicesSchema = Yup.object().shape({
    name: Yup.string()
      .min(3, `${t("minimum3Character")}`)
      .max(50, `${t("maximum50Character")}`)
      .required(`${t("pleaseInputADevicesName")}`),
    email: Yup.string()
      .email()
  ...
  const formikDevices = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        enableLoading();
        await axios.put(`${API_URL}/api/v1/devices/update-development/${devicesId}`, formData);
        handleRefresh();
        disableLoading();
        setAlert("");
        setStatePage("show");
  ...
  const getDevices = async (id) => {
    ...
    try {
      const { data } = await axios.get(`${API_URL}/api/v1/devices/${id}`);
      setDevices({
        name: data.data.name,
      ...
      if (allTransactions.length) {
        ...
        setDevicesHistory(allTransactionsDone);
      }
    } catch (err) {
      setAlert(err.response?.data.message || err.message);
    }
  ...
  const dataTransactions = () => {
    const currHistory = [...devicesHistory];
            ...
            <Paper elevation={2} style={{ padding: "1rem", height: "100%" }}>
              <div className="headerPage">
                ...
                <div className="headerEnd">
                  ...
                  >
                    {statePage === "show" ? `${t("editDevicesData")}` : `${t("cancel")}`}
                  </Button>
              ...
              <Row style={{ padding: "1rem" }}>
                ...
                <Col md={3}>
                  <div className="title">{t("devicesName")}</div>
                  {statePage === "show" ? (
                    <h5 className="mb-5">{formikDevices.values.name}</h5>
                  ) : (
                    <>
                      <Form.Control
                        type="text"
                        name="name"
                        {...formikDevices.getFieldProps("name")}
                        className={validationDevices("name")}
                        required
                      />
                      {formikDevices.touched.name &&
                      formikDevices.errors.name ? (
                  ...
                  <div className="title">{t("devicesEmail")}</div>
                  {statePage === "show" ? (
                    <h5>{formikDevices.values.email}</h5>
                  ) : (
                    <>
                      <Form.Control
                        type="email"
                        name="email"
                        {...formikDevices.getFieldProps("email")}
                        className={validationDevices("email")}
                        required
                      />
                      {formikDevices.touched.email &&
                      formikDevices.errors.email ? (
                        <div className="fv-plugins-message-container">
                          <div className="fv-help-block">
                            {formikDevices.errors.email}
                          </div>
                ...
                <Col md={3}>
                  <div className="title">{t("devicesPhoneNumber")}</div>
                  {statePage === "show" ? (
                    <h5>{formikDevices.values.phone_number}</h5>
                  ) : (
                    <>
                      <Form.Control
                        type="text"
                        name="phone_number"
                        {...formikDevices.getFieldProps("phone_number")}
                        className={validationDevices("phone_number")}
                        required
                      />
                      {formikDevices.touched.phone_number &&
                      formikDevices.errors.phone_number ? (
                        <div className="fv-plugins-message-container">
                          <div className="fv-help-block">
                            {formikDevices.errors.phone_number}
                          </div>
                  ...
                  <div className="title">{t("devicesAddress")}</div>
                  {statePage === "show" ? (
                    <h5>{formikDevices.values.address}</h5>
                  ) : (
                    <>
                      <Form.Control
                        type="text"
                ...
                <Col>
                  <Form.Group>
                    ...
                    <Form.Control
                      ...
                      {...formikDevices.getFieldProps("notes")}
                      className={validationDevices("notes")}
                    />
                  </Form.Group>
                </Col>
              </Row>
            ...
            <Row style={{ padding: "1rem" }}>
              <Col md={3}>
                ...
                <h5 style={{ marginBottom: "2rem" }}>
                  {devicesStats.total_transaction_count
                    ? devicesStats.total_transaction_count
                    : "-"}
                </h5>
                ...
                <h5>
                  {devicesStats.last_visit
                    ? formatDate(devicesStats.last_visit)
                    : "-"}
                </h5>
              </Col>
              ...
              <Col md={3}>
                ...
                <h5 style={{ marginBottom: "2rem" }}>
                  {devicesStats.total_transaction
                    ? <NumberFormat value={devicesStats.total_transaction} displayType={'text'} thousandSeparator={true} prefix={currency} />
                    : "-"}
                </h5>

                <div className="title">{t("favoriteProduct")}</div>
                <h5>
                  {devicesStats.favorite_product
                    ? devicesStats.favorite_product
                    : "-"}
                </h5>
              </Col>
              ...
              <Col md={3}>
                ...
                <h5 style={{ marginBottom: "2rem" }}>
                  {devicesStats.average_transaction
                    ? <NumberFormat value={devicesStats.average_transaction} displayType={'text'} thousandSeparator={true} prefix={currency} />
                    : "-"}
                </h5>

                <div className="title">{t("registrationDate")}</div>
                <h5>
                  {devicesStats.registration_date
                    ? formatDate(devicesStats.registration_date)
                    : "-"}
                </h5>
              </Col>
              ...
              <Col md={3}>
                ...
                <h5 style={{ marginBottom: "2rem" }}>
                  {devicesStats.points ? devicesStats.points : "-"}
                </h5>

                <div className="title">{t("totalPointSpent")}</div>
                <h5>
                  {devicesStats.points_spent
                    ? devicesStats.points_spent
                    : "-"}
                </h5>
              </Col>
